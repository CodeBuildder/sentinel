...
type ReadinessComponent = { name: string; status: 'ready' | 'not_ready' | 'not_configured' | 'not_applicable'; required: boolean; evidence: string; remediation?: string }
type Readiness = { status: string; ready_to_present: boolean; mode: string; checked_at: string; summary: { ready: number; blocking: number; not_applicable: number }; components: ReadinessComponent[] }
...
const provenance = (item?: Pick<Timeline, 'provenance' | 'replayed'>) => {
  const value = item?.provenance || (item?.replayed ? 'replayed' : 'observed')
  if (value === 'simulator') return { value, label: 'SYNTHETIC SIMULATOR' }
  if (value === 'live_chaos' || value === 'chaos_mesh') return { value: 'live_chaos', label: 'LIVE CHAOS MESH' }
  if (value === 'replayed') return { value, label: 'REPLAYED EVIDENCE' }
  if (value === 'observed') return { value, label: 'LIVE OBSERVED' }
  return { value: 'unknown', label: 'UNKNOWN PROVENANCE' }
}
...
function SourceCard({ name, data, icon: Icon }: { name: 'argus' | 'phoenix'; data?: SourceHealth; icon: any }) {
  ...
  return <div className={`source-card ${name} ${connected ? 'connected' : 'waiting'}`}>
    ...
    <span className={`connection ${connected ? 'live' : 'waiting'}`}><i />{connected ? 'evidence connected' : 'waiting for evidence'}</span>
    <div className="source-metrics"><div><b>{data?.findings || 0}</b><small>records</small></div><div><b>{data?.live || 0}</b><small>genuinely live</small></div><div><b>{(data?.critical || 0) + (data?.high || 0)}</b><small>urgent</small></div></div>
  ...
  return <div className="signal-feed">
    ...
    {events.slice(0, 10).map((event, index) => {
      ...
      return <button className={`signal-row ${event.replayed ? 'replayed' : ''}`} key={event.id || index} onClick={() => onSelect(event)}>
        ...
        <em className={`provenance ${provenance(event).value}`}>{provenance(event).label}</em><ChevronRight />
      </button>
    })}
    {!events.length && <Empty text="No operational evidence has reached the Sentinel Operations Graph yet." />}
    <div className="feed-note"><span>Newest evidence across connected agents</span><span>Every record carries one explicit provenance label</span></div>
  </div>
}
...
function ResilienceProof({ incident, onSignal }: { incident: Incident; onSignal: (signal: Timeline) => void }) {
  ...
  const elapsed = (timestamp?: string) => {
    if (!timestamp || !start) return 'not recorded'
    const milliseconds = new Date(timestamp).getTime() - new Date(start).getTime()
    return milliseconds >= 0 ? `+${(milliseconds / 1000).toFixed(milliseconds < 10000 ? 1 : 0)}s` : 'invalid time'
  }
  const metrics = incident.proof?.metrics || {}
  return <section className="proof-section"><h3><Timer /> Resilience proof timeline</h3><p className="timeline-help">Seven required stages, backed by timestamps from the experiment. Missing evidence is shown as unavailable—never inferred.</p>
    <div className="proof-timeline">{proofStages.map((stage, index) => { const item = supplied.get(stage); return <div className={`proof-stage ${item ? 'recorded' : 'missing'}`} key={stage}><span>{index + 1}</span><div><b>{label(stage)}</b><small>{item?.evidence || 'Evidence unavailable for this record'}</small><em>{item?.source || 'no source'} · {elapsed(item?.timestamp)}</em></div><strong>{item ? (item.status || 'verified') : 'unavailable'}</strong></div> })}</div>
    <div className="proof-metrics"><span><small>DETECTION</small><b>{typeof metrics.detection_ms === 'number' ? `${metrics.detection_ms} ms` : 'not measured'}</b></span><span><small>RECOVERY</small><b>{typeof metrics.recovery_ms === 'number' ? `${metrics.recovery_ms} ms` : 'not measured'}</b></span><span><small>AVAILABILITY</small><b>{typeof metrics.availability_percent === 'number' ? `${metrics.availability_percent}%` : 'not measured'}</b></span><span><small>EVIDENCE SOURCE</small><b>{incident.proof?.evidence_source || 'not recorded'}</b></span></div>
    {(incident.timeline || []).length > 0 && <details className="supporting-evidence"><summary>Supporting correlated evidence ({incident.timeline?.length})</summary><div className="lifecycle">{(incident.timeline || []).map((item, index) => <button key={item.id || index} onClick={() => onSignal(item)}><i style={{ background: color(item.source) }} /><div><header><span style={{ color: color(item.source) }}>{item.source}</span><strong>{item.stage ? label(item.stage) : label(item.type)}</strong><time>{age(item.timestamp)}</time></header><b>{item.summary}</b><small className={`provenance-text ${provenance(item).value}`}>{provenance(item).label}{item.action ? ` · action ${label(item.action)}` : ''}{item.outcome ? ` · outcome ${label(item.outcome)}` : ''}</small></div><ChevronRight /></button>)}</div></details>}
  </section>
}
...
function SignalMix({ data }: { data: Overview | null }) {
  ...
  return <div className="mix"><div className="mix-bars">{severities.map(item => <div key={item.name}><span>{item.name}</span><div><i style={{ width: `${(item.value / maximum) * 100}%`, background: color(item.tone) }} /></div><b>{item.value}</b></div>)}</div><div className="evidence-split"><div><b>{data?.counts.observed || 0}</b><span>live observed</span><small>real agent evidence</small></div><div><b>{data?.counts.live_chaos || 0}</b><span>live Chaos Mesh</span><small>real bounded experiment</small></div><div><b>{data?.counts.simulator || 0}</b><span>synthetic simulator</span><small>no cluster mutation</small></div><div><b>{data?.counts.replayed || 0}</b><span>replayed</span><small>evaluation evidence</small></div></div></div>
}
...
function ReadinessPanel({ readiness, expanded, onToggle, onRefresh }: { readiness: Readiness | null; expanded: boolean; onToggle: () => void; onRefresh: () => void }) {
  const ready = Boolean(readiness?.ready_to_present)
  return <section className={`readiness-panel ${ready ? 'ready' : 'blocked'}`}>
    <button className="readiness-summary" onClick={onToggle} aria-expanded={expanded}><span className="readiness-icon">{ready ? <CheckCircle2 /> : <AlertTriangle />}</span><div><small>PRESENTATION PREFLIGHT · {(readiness?.mode || 'checking').toUpperCase()}</small><b>{readiness ? (ready ? 'Ready to present' : `${readiness.summary.blocking} blocking check${readiness.summary.blocking === 1 ? '' : 's'}`) : 'Checking the platform…'}</b><p>Argus · Phoenix · Sentinel · SOG · OpenAI · Kubernetes security and chaos stack</p></div><em>{readiness ? `${readiness.summary.ready} ready · ${readiness.summary.not_applicable} N/A` : 'connecting'} <ChevronRight /></em></button>
    {expanded && <div className="readiness-body"><div className="readiness-head"><p>This panel is read-only. It checks evidence endpoints and cluster readiness without starting services or changing resources.</p><button onClick={onRefresh}><RefreshCw />Check again</button></div><div className="readiness-grid">{(readiness?.components || []).map(item => <article className={item.status} key={item.name}><header><span>{item.status === 'ready' ? <CheckCircle2 /> : item.status === 'not_applicable' ? <CircleDot /> : <AlertTriangle />}</span><div><b>{item.name}</b><small>{label(item.status)}{item.required ? ' · required' : ' · not required in this mode'}</small></div></header><p>{item.evidence}</p>{item.remediation && <em>{item.remediation}</em>}</article>)}</div><footer><span>Last checked {readiness ? age(readiness.checked_at) : 'never'}</span><strong>{ready ? 'All required systems verified' : 'Resolve blocking checks before recording'}</strong></footer></div>}
  </section>
}
...
function HowItWorks({ onClose }: { onClose: () => void }) {
  ...
  return <div className="how-wrap" role="dialog" aria-modal="true" aria-labelledby="how-title"><button className="how-backdrop" onClick={onClose} aria-label="Close How it works" /><article className="how-modal"><header><div><span>THE 30-SECOND SYSTEM STORY</span><h2 id="how-title">Find it early. Break it safely. Prove recovery.</h2><p>Continuous security and resilience testing—before threats spread or customers find the failure.</p></div><button onClick={onClose} aria-label="Close"><X /></button></header>
    ...
    <footer><span><GitBranch />SOG means Sentinel Operations Graph</span><strong>Detect early. Recover before impact. Keep humans in control of high-risk actions.</strong><button onClick={onClose}>Enter command center <ArrowRight /></button></footer>
  </article></div>
}
...
function MetricDetail({ metric, data, onSignal, onComponent, onIncident }: { metric: MetricKind; data: Overview | null; onSignal: (signal: Timeline) => void; onComponent: (component: Component) => void; onIncident: (incident: Incident) => void }) {
  ...
  if (metric === 'signals' || metric === 'urgent') { const rows = metric === 'urgent' ? (data?.timeline || []).filter(item => ['critical', 'high'].includes(item.severity)) : data?.timeline || []; return <><div className="metric-intro"><h2>{definition.title}</h2><p>{definition.description}</p><div><span><b>{rows.length}</b> total</span><span><b>{rows.filter(item => provenance(item).value === 'observed').length}</b> live observed</span><span><b>{rows.filter(item => provenance(item).value === 'live_chaos').length}</b> live chaos</span><span><b>{rows.filter(item => provenance(item).value === 'simulator').length}</b> simulator</span><span><b>{rows.filter(item => provenance(item).value === 'replayed').length}</b> replayed</span></div></div><div className="metric-list">{rows.slice(0, 20).map((item, index) => <button key={item.id || index} onClick={() => onSignal(item)}><i style={{ background: color(item.severity) }} /><div><b>{item.entity_name || label(item.type)}</b><small>{item.source} · {item.summary} · {provenance(item).label}</small></div><strong style={{ color: color(item.severity) }}>{item.severity}</strong><time>{age(item.timestamp)}</time><ChevronRight /></button>)}{!rows.length && <Empty text={`No ${metric === 'urgent' ? 'urgent ' : ''}signals are present.`} />}</div></> }
...
function DetailDrawer({ detail, data, onClose, onDetail }: { detail: Detail; data: Overview | null; onClose: () => void; onDetail: (detail: Detail) => void }) {
  ...
  return <div className="drawer-wrap" role="dialog" aria-modal="true"><button className="drawer-backdrop" onClick={onClose} aria-label="Close details" /><aside className="drawer"><div className="drawer-head"><div><span>{detail.kind === 'signal' ? 'OPERATIONAL EVIDENCE' : 'RESOURCE INTELLIGENCE'}</span><h2>{signal?.entity_name || component?.name || 'Unmapped resource'}</h2><p>{signal?.entity_id || component?.entity_id}</p></div><button onClick={onClose}><X /></button></div>
    ...
    {signal && <div className="signal-summary"><span style={{ color: color(signal.source) }}>{signal.source}</span><strong style={{ color: color(signal.severity) }}>{signal.severity}</strong><em className={`provenance ${provenance(signal).value}`}>{provenance(signal).label}</em><p>{signal.summary}</p></div>}
...
export default function App() {
  ...
  useEffect(() => { loadReadiness(); const timer = setInterval(loadReadiness, 30000); return () => clearInterval(timer) }, [])
